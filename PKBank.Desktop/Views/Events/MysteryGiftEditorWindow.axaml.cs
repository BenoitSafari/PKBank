...
public sealed partial class MysteryGiftEditorWindow : Window
{
    ...
    private void BuildAlbumUi(SaveFile sav)
    {
        ...
        else
        {
            ...
            for (int start = 0; start < _album.Length; start += cellsPerRow)
            {
                int count = Math.Min(cellsPerRow, _album.Length - start);
    ...
    private void SetGift(int index)
    {
        ...
        int lastUnfilled = GetLastUnfilledByType(gift, _album);
